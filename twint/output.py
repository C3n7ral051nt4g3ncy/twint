...
def _clean_follow_list():
    logme.debug(f'{__name__}:clean_follow_list')
    global _follows_object
    _follows_object = {}


def clean_lists():
    logme.debug(f'{__name__}:clean_lists')
    global follows_list
    global tweets_list
    global users_list
    follows_list = []
...
def datecheck(datetimestamp, config):
    logme.debug(f'{__name__}:datecheck')
    if config.Since:
        logme.debug(f'{__name__}:datecheck:SinceTrue')
        ...
        if d < s:
            return False
    if config.Until:
        logme.debug(f'{__name__}:datecheck:UntilTrue')
        ...
        if d > s:
            return False
    logme.debug(f'{__name__}:datecheck:dateRangeFalse')
    return True
...
def is_tweet(tw):
    try:
        tw["data-item-id"]
        logme.debug(f'{__name__}:is_tweet:True')
        return True
    except:
        logme.critical(f'{__name__}:is_tweet:False')
        return False


def _output(obj, output, config, **extra):
    logme.debug(f'{__name__}:_output')
    if config.Lowercase:
        if isinstance(obj, str):
            logme.debug(f'{__name__}:_output:Lowercase:username')
            obj = obj.lower()
...
async def checkData(tweet, config, conn):
    logme.debug(f'{__name__}:checkData')
    tweet = Tweet(tweet, config)
    if not tweet.datestamp:
        logme.critical(f'{__name__}:checkData:hiddenTweetFound')
        print("[x] Hidden tweet found, account suspended due to violation of TOS")
        return
    if datecheck(f"{tweet.datestamp} {tweet.timestamp}", config):
        output = format.Tweet(config, tweet)
        if config.Database:
            logme.debug(f'{__name__}:checkData:Database')
            db.tweets(conn, tweet, config)
        if config.Pandas:
            logme.debug(f'{__name__}:checkData:Pandas')
            panda.update(tweet, config)
        if config.Store_object:
            logme.debug(f'{__name__}:checkData:Store_object')
            if hasattr(config.Store_object_tweets_list, 'append'):
                config.Store_object_tweets_list.append(tweet)
            else:
                tweets_list.append(tweet)
        if config.Elasticsearch:
            logme.debug(f'{__name__}:checkData:Elasticsearch')
...
async def Tweets(tweets, config, conn):
    logme.debug(f'{__name__}:Tweets')
    if config.Favorites or config.Location:
        logme.debug(f'{__name__}:Tweets:fav+full+loc')
        for tw in tweets:
            await checkData(tw, config, conn)
    elif config.TwitterSearch or config.Profile:
        logme.debug(f'{__name__}:Tweets:TwitterSearch')
        await checkData(tweets, config, conn)
    else:
        logme.debug(f'{__name__}:Tweets:else')
...
async def Users(u, config, conn):
    ...
    if config.Database:
        logme.debug(f'{__name__}:User:Database')
        db.user(conn, config, user)

    if config.Elasticsearch:
        logme.debug(f'{__name__}:User:Elasticsearch')
    ...
    if config.Store_object:
        logme.debug(f'{__name__}:User:Store_object')
    ...
    if config.Pandas:
        logme.debug(f'{__name__}:User:Pandas+user')
...
async def Username(username, config, conn):
    ...
    if config.Database:
        logme.debug(f'{__name__}:Username:Database')
        db.follow(conn, config.Username, config.Followers, username)

    if config.Elasticsearch:
        logme.debug(f'{__name__}:Username:Elasticsearch')
    ...
    if config.Pandas:
        logme.debug(f'{__name__}:Username:object+pandas')
        try:
        ...
        if config.Pandas_au:
            logme.debug(f'{__name__}:Username:object+pandas+au')
